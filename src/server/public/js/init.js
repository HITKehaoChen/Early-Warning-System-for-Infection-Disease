...
$(document).ready(() => {
  ...
  $('#form-warning').submit(e => {
    ...
    let population = obj.population.split(' ');
    let prevalence = obj.prevalence.split(' ');

    const postData = {
      ...obj,
      matrix: JSON.stringify(matrix),
      population: JSON.stringify(population),
      prevalence: JSON.stringify(prevalence)
    };
    Axios
      .post('http://localhost:8080/preview/api.do', qs.stringify(postData))
      .then(({ data }) => {
        console.log('api data: ', data);

        const { cur_timeList, ...res } = data;
        let series = [];
        let cityNumber = obj.city_number;
